...
import { InsuranceWarnPanel } from './Insurance';
...
export function BookingWizard() {
  ...
  const [shortageMsg, setShortageMsg] = useState<string | null>(null);
  ...
  const submit = async () => {
    if (!picked) return;
    try {
      const r = await api<{ payType?: string }>('/appointments', { body: { patientId, equipmentId, date: picked.date, start: picked.start, duration: result?.duration || duration } });
      toast(r?.payType === 'self_pay' ? '预约成功（使用家属确认的自费额度）' : '预约成功', 'ok');
      setPicked(null);
      setResult(null);
      setShortageMsg(null);
      await useStore.getState().reload();
      // 重新拉取时段
      const rr = await api<SlotResult>('/appointments/slots', { body: { patientId, equipmentId, duration: duration || undefined } });
      setResult(rr);
    } catch (e) {
      if (e instanceof ApiError && e.code === 'INSURANCE_SHORTAGE') {
        setShortageMsg(e.message);
        await useStore.getState().reload();
      } else {
    ...
    <Section title="预约训练（时段由器械、治疗师排班、患者风险、时长与上轮反馈综合生成）">
      ...
      {patient && (
        <div className="patient-strip">
          ...
          {(patient.insuranceItems[0]?.selfPayTotal ?? 0) > 0 && (
            <Pill tone="purple">自费额度余 {(patient.insuranceItems[0]?.selfPayTotal ?? 0) - (patient.insuranceItems[0]?.selfPayUsed ?? 0)} 次</Pill>
          )}
      ...
      {result?.insurance?.confirmation && <InsuranceWarnPanel c={result.insurance.confirmation} />}
      {shortageMsg && (
        <div className="alert-list tone-red">
          <div>⚠ {shortageMsg}</div>
          <div className="muted">流程：医生在「续开建议」中填写建议 → 家属在「医保确认」中确认自费 → 确认结果同步前台收费与治疗师计划，之后即可按自费预约。</div>
        </div>
      )}
      ...
      {picked && patient && (
        <div className="confirm-bar">
          ...
          <button className="btn btn-primary" onClick={submit}>确认预约</button>
        </div>
      )}
    </Section>
  );
